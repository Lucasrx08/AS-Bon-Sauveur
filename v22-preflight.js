...
const SESSION_KEY='bs-v22-private-session';
...
Storage.prototype.getItem=function(key){
  if(this===localStorage&&key===DATA_KEY){
    ...
    const priv=parse(nativeGet.call(sessionStorage,SESSION_KEY),{});
...
if(existing){
  const {publicData,privateData}=splitData(existing);
  nativeSet.call(localStorage,DATA_KEY,JSON.stringify(publicData));
  nativeSet.call(sessionStorage,SESSION_KEY,JSON.stringify(privateData));
}
...
window.ASV22Privacy={
  ...
  clearPrivateSession(){nativeRemove.call(sessionStorage,SESSION_KEY)}
};
})();