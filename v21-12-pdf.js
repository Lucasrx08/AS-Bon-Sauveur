...
function drawProgramEvent(doc,event,slot){
 ...
 const cols=[{x:tx,w:43,l:'CATÉGORIE',v:event.ageCategory||'Toutes catégories'},{x:tx+47,w:39,l:'HORAIRES',v:`${fmtClock(event.startTime)||'—'}${event.endTime?' - '+fmtClock(event.endTime):''}`},{x:tx+90,w:tw-90,l:'LIEU',v:event.place||'À préciser'}];
...
function drawConvocationFirst(doc,c,students,totalPages){
 ...
 infoBox(doc,'Lieu',c.place||'À préciser',63,68,111,18,theme.accent,{valueSize:12,minValueSize:9,maxLines:1});infoBox(doc,'Départ',fmtClock(c.departure)||'À préciser',179,68,49,18,theme.accent,{valueSize:14,minValueSize:10,maxLines:1});infoBox(doc,'Retour',fmtClock(c.returnTime)||'À préciser',233,68,51,18,theme.accent,{valueSize:14,minValueSize:10,maxLines:1});infoBox(doc,'Point de rendez-vous',c.meetingPoint||'À préciser',63,90,221,17,theme.accent,{valueSize:11.5,minValueSize:9,maxLines:1});
