...
function specialtyHome(){
 ...
 document.querySelector('.v10-specialty-note')?.remove();const n=V.read().specialtyNotes?.[sp],ok=n?.active&&String(n.message||'').trim()&&(!n.expiresAt||n.expiresAt>=V.today());
 if(ok&&hero){const x=document.createElement('section');x.className='v10-specialty-note';x.innerHTML=`<span class="v10-note-icon">${I.note}</span><div><b>À RETENIR</b><p>${V.esc(n.message)}</p>${n.expiresAt?`<small>Jusqu’au ${new Intl.DateTimeFormat('fr-FR').format(new Date(n.expiresAt+'T12:00:00'))}</small>`:''}</div>`;hero.insertAdjacentElement('afterend',x)}
}
...
refresh();window.ASV10={icons:I,refresh,version:'v10-20260907-1'};
})();
