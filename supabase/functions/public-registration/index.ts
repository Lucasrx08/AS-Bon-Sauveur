...
Deno.serve(async request=>{
 ...
 try{
  ...
  const {data:existing,error:existingError}=await admin.from('v20_event_registrations').select('id,created_at').eq('request_id',requestId).maybeSingle();
  if(existingError)throw existingError;
  if(existing)return json(request,{status:'registered',createdAt:existing.created_at},200);
  const ipHash=await hashKey(serviceKey,'registration-ip:'+clientIp(request));
  const identityHash=await hashKey(serviceKey,'registration-person:'+eventId+'|'+normalizeLogin(lastName)+'|'+normalizeLogin(firstName)+'|'+className);
  ...
  const {data:event,error:eventError}=await admin.from('v20_events').select('id,title,specialty,date,public_visible,registration_open,convocation_id').eq('id',eventId).maybeSingle();
  ...
  const {data:linked,error:linkedError}=await admin.from('v20_convocations').select('id').eq('date',event.date).eq('specialty',event.specialty).eq('title',event.title).eq('public_visible',true).eq('status','published').limit(1);
  if(linkedError)throw linkedError;
  if(Array.isArray(linked)&&linked.length)return json(request,{error:'Les inscriptions libres sont fermées pour cet événement.'},409);
  const due=new Date(String(event.date)+'T12:00:00Z');due.setUTCDate(due.getUTCDate()+90);
  const {data:created,error:createError}=await admin.from('v20_event_registrations').insert({id:requestId,request_id:requestId,event_id:eventId,last_name:lastName,first_name:firstName,class_name:className,retention_due_at:due.toISOString()}).select('created_at').single();
  if(createError){
   if(createError.code==='23505')return json(request,{error:'Cet élève est déjà inscrit à cet événement.'},409);
   throw createError;
  }
  ...
  console.error('public-registration',error instanceof Error?error.name:'error');
  return json(request,{error:'L’inscription n’a pas pu être enregistrée. Réessayez.'},500);
 }
});
