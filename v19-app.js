...
function manageTerms(){
 ...
 m.querySelector('#v19-terms-form').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,fd=new FormData(form),next={};[1,2,3].forEach(t=>next[t]={deadline:fd.get(`d${t}`)||'',end:fd.get(`e${t}`)||''});const button=form.querySelector('button[type="submit"]');button.disabled=true;button.textContent='Enregistrement…';try{if(typeof window.__BS_PERSIST_TERMS!=='function')throw new Error('SERVICE_UNAVAILABLE');await window.__BS_PERSIST_TERMS(next);state.data.termSettings=next;save();closeModal();render();(window.__BS_SAVED?window.__BS_SAVED('Dates enregistrées'):toast('Dates des trimestres enregistrées dans la base centrale.'))}catch(error){console.error('Trimestres',error);button.disabled=false;button.textContent='Enregistrer';toast('Dates non enregistrées : vérifiez votre connexion puis réessayez.',4800)}};
}
