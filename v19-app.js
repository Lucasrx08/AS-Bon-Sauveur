...
function appreciationReviewPage(){
 ...
 return `<div class="v19-container">${pageTitle('SUIVI PÉDAGOGIQUE','Appréciations','Espace administrateur · suivi des élèves de Football, Escalade et Sport-études Gymnastique.',`<button class="v19-btn secondary" onclick="app.refreshAppreciationReview()">Actualiser</button>`)}
...
function editReport(id){
 ...
 const m=modal(id?'Modifier un bilan':'Ajouter un bilan',`<form id="v19-report-form" class="v19-form">
  ...
  <div class="full v19-modal-actions"><button class="v19-btn" type="submit">Enregistrer</button></div></form>`,true);
 const form=m.querySelector('#v19-report-form'),button=form.querySelector('button[type="submit"]');
 form.onsubmit=async ev=>{ev.preventDefault();const fd=new FormData(form),teachers=[...m.querySelectorAll('.v19-teacher-picker input:checked')].map(x=>x.value);if(!teachers.length)return alert('Choisissez au moins un enseignant référent.');const o={id:r?.id||uid('b'),date:fd.get('date'),activity:fd.get('activity'),teacher:teachers.join(' · '),level:fd.get('level'),place:String(fd.get('place')||'').trim(),category:fd.get('category'),participants:Number(fd.get('participants')||0),comment:String(fd.get('comment')||'').trim()};button.disabled=true;button.textContent='Enregistrement…';try{if(typeof window.__BS_PERSIST_REPORT!=='function')throw new Error('SERVICE_UNAVAILABLE');const saved=await window.__BS_PERSIST_REPORT(o);const finalReport={...o,...saved};r?Object.assign(r,finalReport):state.data.reports.push(finalReport);save();closeModal();render();toast('Bilan enregistré dans la base centrale.')}catch(error){console.error('Enregistrement bilan',error);button.disabled=false;button.textContent='Enregistrer';toast('Bilan non enregistré : vérifiez votre connexion puis réessayez.',4800)}};
}
...
async function saveSpecialtyNote(){
 const sp=document.querySelector('#v19-note-sp')?.value;if(!sp||!isAdmin())return;const note={message:String(document.querySelector('#v19-note-msg')?.value||'').trim(),expiresAt:document.querySelector('#v19-note-exp')?.value||'',active:!!document.querySelector('#v19-note-active')?.checked};try{if(typeof window.__BS_PERSIST_SPECIALTY_NOTE!=='function')throw new Error('SERVICE_UNAVAILABLE');const saved=await window.__BS_PERSIST_SPECIALTY_NOTE(sp,note);state.data.specialtyNotes[sp]={...note,...saved,message:saved.message??note.message,expiresAt:saved.expiresAt??note.expiresAt,active:saved.active??note.active};save();closeModal();toast('Post-it enregistré dans la base centrale.')}catch(error){console.error('Post-it',error);toast('Post-it non enregistré : vérifiez votre connexion puis réessayez.',4800)}
}
function manageTerms(){
 ...
 m.querySelector('#v19-terms-form').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,fd=new FormData(form),next={};[1,2,3].forEach(t=>next[t]={deadline:fd.get(`d${t}`)||'',end:fd.get(`e${t}`)||''});const button=form.querySelector('button[type="submit"]');button.disabled=true;button.textContent='Enregistrement…';try{if(typeof window.__BS_PERSIST_TERMS!=='function')throw new Error('SERVICE_UNAVAILABLE');await window.__BS_PERSIST_TERMS(next);state.data.termSettings=next;save();closeModal();render();toast('Dates des trimestres enregistrées dans la base centrale.')}catch(error){console.error('Trimestres',error);button.disabled=false;button.textContent='Enregistrer';toast('Dates non enregistrées : vérifiez votre connexion puis réessayez.',4800)}};
}
...
async function downloadLicenseTemplate(){
 ...
 ws.mergeCells('A1:D1');const title=ws.getCell('A1');title.value='IMPORT LICENCIÉS — AS BON SAUVEUR';title.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF1460CC'}};title.font={name:'Aptos Display',size:24,bold:true,color:{argb:'FFFFFFFF'}};title.alignment={horizontal:'left',vertical:'middle'};ws.getRow(1).height=38;
 ws.mergeCells('A2:D2');const sub=ws.getCell('A2');sub.value='ASSOCIATION SPORTIVE DU BON SAUVEUR · SAINT-LÔ';sub.font={name:'Aptos',size:11,color:{argb:'FF13213A'}};sub.alignment={vertical:'middle'};ws.getRow(2).height=24;
 ['Nom & prénom','Classe','Catégorie','Mode de paiement'].forEach((h,i)=>{const cell=ws.getCell(4,i+1);cell.value=h;cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFFFD21A'}};cell.font={name:'Aptos',size:12,bold:true,color:{argb:'FF13213A'}};cell.alignment={horizontal:'center',vertical:'middle'};cell.border={bottom:{style:'thin',color:{argb:'FF1460CC'}}}});ws.getRow(4).height=28;
 ws.getCell('A5').value='MARTIN Léo';ws.getCell('B5').value='5e Jacqueline AURIOL';ws.getCell('C5').value='Benjamin';ws.getCell('D5').value='Chèque';
 ws.getColumn(1).width=32;ws.getColumn(2).width=32;ws.getColumn(3).width=22;ws.getColumn(4).width=24;
 ...
 for(let r=5;r<=200;r++){
  ws.getCell(r,2).dataValidation={type:'list',allowBlank:true,formulae:[`$X$2:$X$${CLASSES.length+1}`]};
  ws.getCell(r,3).dataValidation={type:'list',allowBlank:true,formulae:[`$Y$2:$Y$${LICENSE_CATEGORIES.length+1}`]};
  ws.getCell(r,4).dataValidation={type:'list',allowBlank:true,formulae:[`$Z$2:$Z$${PAYMENTS.length+1}`]};
 }
 const help=wb.addWorksheet('Aide');help.getCell('A1').value='CATÉGORIES AUTORISÉES';LICENSE_CATEGORIES.forEach((x,i)=>help.getCell(i+2,1).value=x);help.getCell('C1').value='CLASSES AUTORISÉES';CLASSES.forEach((x,i)=>help.getCell(i+2,3).value=x);help.getCell('E1').value='MODES DE PAIEMENT LICENCES';PAYMENTS.forEach((x,i)=>help.getCell(i+2,5).value=x);help.getColumn(1).width=24;help.getColumn(3).width=34;help.getColumn(5).width=28;
