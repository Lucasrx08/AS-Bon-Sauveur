...
const periodLabel=rows=>{
  ...
  const a=dateParts(sorted[0].date),b=dateParts(sorted[sorted.length-1].date);
...
function wrapLines(ctx,text,maxWidth,maxLines=2){
  ...
  if(consumed<words.length&&lines.length){let last=lines[lines.length-1];while(last.length>1&&ctx.measureText(last+'…').width>maxWidth)last=last.slice(0,-1);lines[lines.length-1]=last.replace(/[\s,.;:-]+$/,'')+'…'}
  return lines;
}
function alphaColor(hex,alpha=.09){
  const value=String(hex||'').replace('#','');
  if(!/^[0-9a-fA-F]{6}$/.test(value)) return `rgba(45,79,170,${alpha})`;
...
async function loadImage(src){return await new Promise((resolve,reject)=>{const img=new Image();img.onload=()=>resolve(img);img.onerror=reject;img.src=src+(src.includes('?')?'&':'?')+'v=23.0';})}
async function buildTvCanvas(rows){
  ...
  rows.forEach((event,i)=>{
    ...
    const pillsX=x+w-32-specW-catW-12;drawPill(ctx,category,pillsX,metaY-31,'#F3F6FA','#586982');drawPill(ctx,speciality,pillsX+catW+12,metaY-31,alphaColor(accent,.09),accent);
  });
  return canvas;
}

function closePicker(){document.getElementById('v23-tv-calendar-modal')?.remove()}
function exportTv(){
  const rows=events();if(!rows.length)return alert('Aucun événement à exporter.');
  closePicker();
  const wrapper=document.createElement('div');wrapper.id='v23-tv-calendar-modal';wrapper.className='v19-modal-backdrop v2112-export-backdrop';
  wrapper.innerHTML=`<div class="v19-modal wide v2112-export-modal" role="dialog" aria-modal="true" aria-labelledby="v23-tv-export-title"><div class="v19-modal-head"><div><div class="v19-kicker">EXPORT TV</div><h2 id="v23-tv-export-title">Créer le programme écran</h2></div><button class="v19-icon-btn" type="button" aria-label="Fermer" data-close>×</button></div><form class="v19-modal-body"><div class="v2112-picker-layout"><section><div class="v2112-picker-intro"><div><strong>Choisissez jusqu’à ${MAX_EVENTS} dates</strong><p>Le visuel sera généré en PNG 16:9 avec ton fond TV.</p></div><span class="v2112-count" data-count>0 / ${MAX_EVENTS}</span></div><div class="v2112-event-list">${rows.map(e=>{const p=dateParts(e.date);return `<label class="v2112-event-choice"><input type="checkbox" name="eventIds" value="${esc(e.id)}"><span class="v2112-choice-date"><b>${esc(p.day)}</b><small>${esc(p.monthShort)}</small></span><span class="v2112-choice-main"><strong>${esc(e.title||'Rendez-vous')}</strong><small>${esc(time(e.startTime)||'—')}${e.endTime?' - '+esc(time(e.endTime)):''} · ${esc(e.place||'À préciser')}</small><em>${esc(e.ageCategory||'Toutes catégories')} · ${esc(e.specialty||'Association Sportive')}</em></span></label>`}).join('')}</div><div class="v2112-limit" data-limit aria-live="polite"></div></section><aside class="v2112-preview"><div class="v23-tv-preview-sheet"><div class="v23-tv-preview-content" data-preview></div></div><span>Aperçu TV 16:9</span></aside></div><div class="v19-modal-actions v2112-actions"><button class="v19-btn secondary" type="button" data-cancel>Annuler</button><button class="v19-btn v23-tv-btn" type="submit" data-export>Exporter le PNG</button></div></form></div>`;
  ...
  if(typeof canvas.toBlob==='function'){canvas.toBlob(b=>b?resolve(b):reject(new Error('PNG')),'image/png');return}
  try{
    const dataUrl=canvas.toDataURL('image/png'),bytes=atob(dataUrl.split(',')[1]),array=new Uint8Array(bytes.length);
    for(let i=0;i<bytes.length;i++)array[i]=bytes.charCodeAt(i);
    resolve(new Blob([array],{type:'image/png'}));
  }catch(err){reject(err)}
});downloadBlob(blob,`Programme_TV_AS_${clean(periodLabel(chosen))}.png`);closePicker()}catch(error){console.error(error);button.disabled=false;button.textContent='Exporter le PNG';limit.textContent='La création du visuel TV a échoué. Réessayez.';limit.classList.add('error')}};
