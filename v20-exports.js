...
window.app.exportCalendarPDF=async()=>{try{await ensureDeps();const {jsPDF}=window.jspdf||{};if(!jsPDF)throw new Error('Module PDF indisponible');const d=window.app.readData(),sp=window.app.roleSpecialty();let events=(d.events||[]).filter(e=>String(e.date||'')>=new Date().toISOString().slice(0,10)).filter(e=>!sp||e.specialty===sp).slice().sort((a,b)=>(a.date+(a.startTime||'')).localeCompare(b.date+(b.startTime||''))).slice(0,3);if(!events.length)return alert('Aucun événement à exporter.');const cv=await programCanvas(events);const doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a3',compress:true});doc.addImage(cv.toDataURL('image/png'),'PNG',0,0,297,420,'','FAST');doc.save(sp?`Programme_${clean(sp)}.pdf`:'Programme_AS_Bon_Sauveur.pdf')}catch(e){console.error(e);alert('Impossible de générer le calendrier PDF.')}};
