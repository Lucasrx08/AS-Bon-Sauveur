...
function info(ctx,label,value,x,y,w,{accent=C.blue,valueSize=30,maxLines=2}={}){txt(ctx,label.toUpperCase(),x,y,16,900,accent);block(ctx,value||'—',x,y+36,w,valueSize,19,maxLines,800,C.ink)}
function chip(ctx,label,x,y,bg='#EEF4FF',fg=C.blue,maxW=400){ctx.font=`800 20px ${bodyFont}`;const w=Math.min(maxW,ctx.measureText(label).width+36);rr(ctx,x,y,w,44,22,bg);txt(ctx,label,x+18,y+29,20,800,fg);return w}
...
async function convCanvas(c,studentNames){
 ...
 rr(ctx,70,470,305,300,28,'#F4F7FB');txt(ctx,dp.weekday,95,525,20,900,a);txt(ctx,dp.day,95,675,115,400,a,'left',titleFont);ctx.fillStyle=C.yellow;ctx.fillRect(95,700,180,10);txt(ctx,dp.month,95,754,27,400,a,'left',titleFont);
 ...
 txt(ctx,'ÉLÈVES CONVOQUÉS',440,930,22,900,C.blue);ctx.fillStyle='#BFD1EA';ctx.fillRect(440,946,330,4);
 const shown=studentNames.slice(0,24),cols=3,cw=485,startY=985;shown.forEach((n,i)=>{const col=i%cols,row=Math.floor(i/cols),xx=440+col*cw,yy=startY+row*62;rr(ctx,xx,yy,440,48,24,'#F1F4F8');block(ctx,n,xx+18,yy+32,405,21,16,1,800,C.ink)});
 if(studentNames.length>24)txt(ctx,`+ ${studentNames.length-24} élève(s) supplémentaire(s)`,440,1372,18,800,C.muted);
 return cv
}
async function convContinuation(c,studentNames,page,total){const im=await logo(),{c:cv,ctx}=newCanvas(2048,1448);drawConvDecor(ctx,2048,1448,im);block(ctx,`${c.title||'Convocation'} — ÉLÈVES CONVOQUÉS`,70,360,1550,54,34,2,400,C.blue,titleFont);txt(ctx,`PAGE ${page}/${total}`,1900,355,17,900,C.muted,'right');const cols=4,cw=470,startY=470;studentNames.forEach((n,i)=>{const col=i%cols,row=Math.floor(i/cols),x=70+col*cw,y=startY+row*82;rr(ctx,x,y,420,60,20,'#F4F7FB','#DCE4EE',2);block(ctx,n,x+20,y+39,380,22,16,1,800,C.ink)});return cv}
